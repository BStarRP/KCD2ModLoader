...
namespace big
{
	...
	bool renderer::hook()
	{
		...
		auto hr_res = d3d12_create_device(nullptr, feature_level, IID_PPV_ARGS(&device));

		if (FAILED(hr_res))
		{
			LOG(ERROR) << "Failed to create D3D12 Dummy device";

			LOG(ERROR) << "Reason: " << HEX_TO_UPPER(hr_res);
		}
		...
		auto init_dummy_window = [&]()
		{
			...
			wc.lpszClassName = TEXT("KCD2ML_DX12_DUMMY");
			wc.hIconSm       = NULL;

			::RegisterClassEx(&wc);

			hwnd = ::CreateWindow(wc.lpszClassName, TEXT("KCD2ML Dummy Window"), WS_OVERLAPPEDWINDOW, 0, 0, 100, 100, NULL, NULL, wc.hInstance, NULL);
